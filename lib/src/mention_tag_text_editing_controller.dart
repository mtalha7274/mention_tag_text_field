...
class MentionTagTextEditingController extends TextEditingController {
  ...
  String getSelectionText({bool cut = false}) {
    ...
    final beforeSectionMentionCount =
        beforeSelectionText.countChar(Constants.mentionEscape);

    final selectionText = super.text.substring(startIndex, finalIndex);
    final sectionMentionCount =
        selectionText.countChar(Constants.mentionEscape);

    final tempList = _mentions.sublist(beforeSectionMentionCount);

    if (cut) {
      _mentions.removeRange(beforeSectionMentionCount,
          beforeSectionMentionCount + sectionMentionCount);
